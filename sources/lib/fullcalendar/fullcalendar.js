...
Grid.mixin({
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			...
			for (i = 0; i < segs.length; i++) {
				html += this.fgSegHtml(segs[i], disableResizing, i);
			}
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing, index) {
		var view = this.view;
		var event = seg.event;
		...
		var timeText;
		var titleHtml;
		var id = 'id=' + 'sp-task-id-' + event.TaskID + '-' + index;
		...
		if (event.IsUrgent) {
		    titleHtml = '<span class="fa fa-exclamation-triangle" style="font-size: .85em; margin: .25em 0em 0 0; position: absolute; right: .5em;"></span>' +
			'<span class="fc-title">' +
		...
		return '<a class="' + classes.join(' ') + '"' + id +
				(event.url ?
			...
			'>' +
				'<div class="fc-content truncate pr-3">' +
					(this.isRTL ?
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing, index) {
        ...
        if (event.IsUrgent) {
            iconHtml = '<span class="fa fa-exclamation-triangle" style="position:absolute; margin-top: .25em; right:0; display:block; font-size: .85em; top:0;"></span>';
        }
...
Calendar.defaults = {
	...
	nextDayThreshold: '00:00:00', // 12am
			...
			'<table>' +
				...
				'<tbody class="fc-body">' +
					'<tr>' +
						'<td class="' + this.widgetContentClass + '">' +
							(this.dayGrid ?
								'<div class="fc-day-grid" id="dayExpand"/>' +
